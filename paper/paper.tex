...
Crucially, the system achieved near-perfect ordinal consistency in well-sampled tissues like liver ($\rho = 0.97$) and muscle ($\rho = 0.97$) (\figref{fig:performance}c). The confusion matrices (\figref{fig:performance}b) reveal that ``errors'' largely occurred between adjacent stages (e.g., Early vs. Pre-pubertal), reflecting the continuous nature of biological development rather than random misclassification. Unlike static morphological grading, our probabilistic calibration captures these transitional states.
