...
To assess whether the multi-project nature of the PigGTEx dataset inflates these performance estimates, we conducted a batch structure analysis (see Methods). PERMANOVA revealed that BioProject identity explains substantially more transcriptomic variance than developmental stage in several tissues (e.g., brain: $R^2_{\mathrm{BioProject}} = 0.76$ vs.\ $R^2_{\mathrm{Stage}} = 0.18$; lung: $R^2_{\mathrm{BioProject}} = 0.82$ vs.\ $R^2_{\mathrm{Stage}} = 0.03$; Supplementary Table~S4). Leave-One-Project-Out (LOPO) cross-validation, which holds out entire BioProjects as independent test sets, produced substantially lower balanced accuracy than standard $K$-fold for most tissues (brain: LOPO~BA = 0.09 vs.\ $K$-fold~BA = 0.74; liver: LOPO~BA = 0.34 vs.\ $K$-fold~BA = 0.94; lung: LOPO~BA = 0.61 vs.\ $K$-fold~BA = 0.89; blood: LOPO~BA = 0.52 vs.\ $K$-fold~BA = 0.91; Supplementary Table~S4). These gaps indicate that standard $K$-fold estimates are optimistic because samples from the same BioProject share technical characteristics. The LOPO results represent a more conservative, and arguably more realistic, estimate of generalisation to truly independent datasets. We note that batch correction was not applied prior to modelling (see Discussion for further consideration of this limitation).
...
Several limitations must be acknowledged. First, and most critically, batch effects represent a significant confound in this multi-project meta-analysis. Our batch diagnostics reveal that BioProject identity explains substantially more transcriptomic variance than developmental stage across all tissues tested (PERMANOVA $R^2_{\mathrm{BioProject}}$ = 0.51--0.82 vs.\ $R^2_{\mathrm{Stage}}$ = 0.03--0.18), and Leave-One-Project-Out cross-validation produces dramatically lower balanced accuracy than standard $K$-fold (performance gaps of 28--65\%). Cram\'{e}r's $V$ values approaching 1.0 for several tissues (lung: $V$ = 1.0; blood: $V$ = 0.97; brain: $V$ = 0.87) indicate near-complete confounding between BioProject and Stage, meaning the model cannot fully disentangle biological signal from technical artefacts in these tissues. No batch correction (e.g., ComBat-seq) was applied prior to modelling, as the degree of confounding makes post-hoc correction unreliable for the most affected tissues. The LOPO results therefore represent a more honest assessment of true generalisation performance than the standard $K$-fold estimates reported as primary metrics. Future work should prioritise acquisition of data from diverse technical sources spanning all developmental stages to deconfound batch and biology, and evaluate formal batch correction methods where confounding structure permits.
