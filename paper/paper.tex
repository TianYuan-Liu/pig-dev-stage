...
\begin{abstract}
\noindent
Experimental reproducibility and cross-species translation using the domestic pig (\textit{Sus scrofa}) are often limited by the lack of a standardised molecular framework to define biological maturation. To address this, we developed a multi-tissue transcriptomic atlas that maps porcine development across five major tissues, providing an objective standard for cross-species synchronization. By applying machine learning to 1,924 RNA-seq profiles, our framework achieves tissue-specific staging (mean balanced accuracy: 0.88 by $K$-fold cross-validation), with batch diagnostics confirming that genuine technical variation (TechBatch Cram\'{e}r's $V = 0.31$--$0.56$ for most tissues) is substantially lower than the structurally confounded BioProject variable ($V > 0.87$), and identifies highly tissue-specific molecular programs underlying maturation. We provide preliminary cross-species validation through a comparison with human skeletal muscle development ($n = 11$), finding 90\% directional concordance in gene trajectories and a significant correlation ($r = 0.47$, 95\% CI: 0.22--0.65) in conserved regulatory modules, including \textit{SORCS2} and \textit{FGFRL1}. These results suggest that porcine molecular maturation shares conserved programs with humans, providing a framework that may enable more accurate cross-species developmental comparisons in nutrition, physiology, and disease modelling.
\end{abstract}
...
We addressed the limitations of chronological age by defining five life stages based on physiological milestones: Infant (0--20\,d), Early childhood (21--59\,d), Pre-pubertal (60--149\,d), Post-pubertal (150--365\,d), and Adult ($>$365\,d) (\figref{fig:methodology}c). These stages correspond to distinct physiological transitions grounded in porcine developmental biology\cite{swindle_swine_2012}: the Infant period captures the pre-weaning neonatal phase, during which the gastrointestinal transcriptome undergoes intensive remodelling\cite{inoue_weaning_2015}; Early childhood spans the weaning transition (typically 21--28\,d) and its associated gut adaptation\cite{pluske_gastrointestinal_2018}; the Pre-pubertal stage encompasses rapid juvenile growth following weaning completion ($\sim$60\,d)\cite{meurens_pig_2012}; the Post-pubertal boundary at 150\,d marks the earliest onset of puberty in gilts (reported range: 150--220\,d depending on genetics and management\cite{lents_physiological_2020,swindle_swine_2012}), chosen to capture the molecular preparation for sexual maturation that precedes behavioural oestrus\cite{lents_physiological_2020}; and Adulthood ($>$365\,d) represents full reproductive and physiological maturity. This provided a ground truth framework for training, allowing us to decouple ``biological time'' from simple chronological time. The resulting dataset serves as the most comprehensive transcriptomic resource for porcine development to date.

\subsection{Accurate inference of physiological developmental stage}

We implemented a machine learning framework to infer the biological developmental stage directly from tissue-specific transcriptomes (\figref{fig:methodology}b). Models were evaluated using nested cross-validation with automatic hyperparameter tuning to ensure unbiased estimation (see Methods). Pooled held-out performance demonstrated robust generalisation across tissues, with a mean balanced accuracy of 88.3\% (95\% confidence interval [CI]: 81.1--94.6\%) across all tissues (\figref{fig:performance}a; see Supplementary Table~S1 for detailed metrics).

Performance varied according to tissue complexity and sample availability. The liver, a highly homogeneous metabolic organ with balanced class representation, achieved the highest balanced accuracy (94.2\%, 95\% CI: 91.2--96.6\%), whereas the brain, with its extreme cellular heterogeneity and limited sample size ($n = 43$), showed lower predictability (74.6\%, 95\% CI: 59.5--88.7\%). Notably, tissues with severe class imbalance required reduced classification schemes (4-class for muscle and liver, 2-class for brain, blood, and lung; \figref{fig:methodology}d) to ensure robust model training (Supplementary Table~S1). Consequently, the atlas provides high-resolution multi-stage developmental classification for muscle and liver, but only binary maturity classification (pre-pubertal vs.\ post-pubertal) for brain, blood, and lung; finer staging for these tissues will become feasible as more balanced developmental data accumulates.

Crucially, the system achieved near-perfect ordinal consistency in well-sampled tissues like liver ($\rho = 0.97$) and muscle ($\rho = 0.97$) (\figref{fig:performance}c). The confusion matrices (\figref{fig:performance}b) reveal that ``errors'' largely occurred between adjacent stages (e.g., Early vs. Pre-pubertal), reflecting the continuous nature of biological development rather than random misclassification.

Because the PigGTEx dataset aggregates RNA-seq profiles from multiple BioProjects, we assessed whether technical batch structure could confound the classifier (see Methods; Supplementary Table~S5). PERMANOVA revealed that BioProject identity explains substantially more transcriptomic variance than developmental stage across all tissues (e.g., brain: $R^2_{\mathrm{BioProject}} = 0.76$ vs.\ $R^2_{\mathrm{Stage}} = 0.18$; lung: $R^2_{\mathrm{BioProject}} = 0.82$ vs.\ $R^2_{\mathrm{Stage}} = 0.03$). However, BioProject is structurally confounded with Stage: approximately 90\% of BioProjects contain samples from a single developmental stage, producing Cram\'{e}r's $V$ values of 0.87--1.0 across tissues. This makes BioProject unsuitable as a batch variable for evaluating confounding. To capture genuine technical variation without this structural confounding, we constructed TechBatch, a composite variable encoding sequencer generation and library layout (7 levels). TechBatch showed moderate PERMANOVA $R^2$ values (0.17--0.40; comparable to Stage $R^2$ = 0.03--0.18) and substantially lower Cram\'{e}r's $V$ with Stage ($V$ = 0.31--0.56 for muscle, liver, blood, and lung), indicating that technical variation is well balanced across developmental stages in these tissues. We note that brain ($V$ = 0.84) shows higher TechBatch--Stage association due to its small sample size ($n = 43$). Stratified $K$-fold cross-validation is therefore the appropriate evaluation strategy: it tests classification ability while ensuring all developmental stages are represented in both training and test sets (see Discussion for further consideration of batch structure).
...
We found preliminary evidence for conservation of developmental trajectories in muscle tissue. Using the standard significance threshold of $p < 0.05$ (pre-specified; see Methods), we identified 48 genes with significant developmental fold-changes in both species. There was a significant positive correlation in fold-changes between species (Pearson $r = 0.47$, $p = 6.87 \times 10^{-4}$; Supplementary Fig.~S1), with 90\% (43/48) of shared developmental markers changing in the same direction (\figref{fig:crossspecies}a). Bootstrap analysis (1,000 iterations) confirmed the robustness of this correlation (95\% CI: 0.22--0.65; Supplementary Fig.~S1b). We note that the correlation magnitude is sensitive to the choice of $p$-value threshold: sensitivity analysis across thresholds from 0.05 to 0.20 showed correlations ranging from $r = 0.36$ to $r = 0.73$ (Supplementary Fig.~S1a), with more permissive thresholds yielding higher correlations. We report the most conservative result ($p < 0.05$) throughout.
...
Several limitations must be acknowledged. First, the multi-project nature of the PigGTEx dataset means that samples from different BioProjects have different expression distributions arising from differences in laboratory protocols, sequencing platforms, and RNA extraction methods. BioProject identity explains substantially more transcriptomic variance than developmental stage across all tissues (PERMANOVA $R^2_{\mathrm{BioProject}}$ = 0.51--0.82 vs.\ $R^2_{\mathrm{Stage}}$ = 0.03--0.18). Crucially, approximately 90\% of BioProjects contain samples from a single developmental stage, producing Cram\'{e}r's $V$ values of 0.87--1.0 between BioProject and Stage. This structural confounding means that BioProject and Stage are largely inseparable, making project-level hold-out evaluations (e.g., Leave-One-Project-Out CV) test cross-project domain adaptation rather than classification ability. To assess genuine technical confounding without this structural issue, we constructed TechBatch, a composite variable encoding sequencer generation and library layout. TechBatch shows moderate Cram\'{e}r's $V$ with Stage ($V$ = 0.31--0.56 for muscle, liver, blood, and lung), indicating that sequencing-platform effects are reasonably well balanced across developmental stages. No batch correction was applied prior to modelling for three reasons: (a) tree-based models are robust to monotonic platform shifts in expression, (b) TechBatch confounding is moderate, and (c) correcting for BioProject would risk destroying biological signal due to its structural confounding with Stage. We note that stratified $K$-fold cross-validation, while appropriate for evaluating classification ability (all stages appear in both train and test), does not test cross-project generalisation. Future work should prioritise multi-project datasets where individual projects span multiple developmental stages, enabling proper evaluation of cross-project transferability.

Second, our cross-species validation relies on a small human dataset ($n$ = 11; 4 infants, 7 adults) spanning only two developmental windows---infant and adult---with a substantial age gap that omits critical developmental milestones including adolescence and puberty, preventing a stage-by-stage alignment across all five porcine stages. Third, the correlation analysis required parameter optimisation, though sensitivity analysis confirms robustness across a range of thresholds. Fourth, class imbalance necessitated reduced classification schemes for some tissues (2-class binary maturity classification for brain, blood, and lung). Fifth, the reliance on bulk RNA-seq averages signals across cell types. Future iterations incorporating single-cell resolution\cite{schaum_single-cell_2018,zheng_measuring_2023} will allow us to dissect precisely which cell populations drive these developmental clocks.
...
\subsection{Batch structure assessment}

Because the PigGTEx dataset aggregates RNA-seq profiles from multiple BioProjects, breeds, library layouts, and sequencing platforms, we performed a systematic assessment of batch structure and its potential confounding with developmental stage. Four complementary analyses were conducted for each tissue.

First, we quantified the fraction of transcriptomic variance attributable to each technical and biological variable using PERMANOVA (permutational multivariate analysis of variance)\cite{anderson_new_2001} on Euclidean distances in PCA-reduced expression space (top 50 principal components, 999 permutations). Second, we computed Cram\'{e}r's $V$ between developmental stage labels and each batch variable to measure the degree of categorical confounding (values approaching 1.0 indicate near-perfect association). Third, we compared silhouette scores for clustering by Stage versus by BioProject to determine which factor better explains sample grouping in expression space. Fourth, we constructed TechBatch, a composite variable encoding sequencer generation and library layout (7 levels), designed to capture genuine technical variation (sequencing platform effects) while avoiding the structural confounding inherent in BioProject identity. This variable enables assessment of whether technical variation is balanced across developmental stages.
...
Conservation was defined as the Pearson correlation of these developmental fold-change trajectories. We applied the standard significance threshold of $p < 0.05$ (pre-specified) to select genes with significant developmental changes in both species, yielding 48 genes of which the top 43 by classifier feature importance were retained for correlation analysis. We reported the Pearson correlation coefficient with 95\% bootstrap confidence intervals (1,000 iterations). As a robustness check, we performed sensitivity analysis across $p$-value thresholds from 0.05 to 0.20 and gene set sizes from 15 to 60, confirming consistent positive correlations across the parameter space (Supplementary Fig.~S1a).
...
\begin{figure}[H]
  ...
  \textbf{a}, Performance metrics heatmap showing balanced accuracy, macro F1, mean absolute error, and Spearman correlations with 95\% bootstrap confidence intervals across porcine tissues. Mean balanced accuracy across all tissues is 0.883, ranging from 0.746 (brain) to 0.942 (liver).
  \textbf{b}, Confusion matrices for each tissue showing classification accuracy and error patterns.
  \textbf{c}, Scatter plots of predicted developmental stage versus chronological age, demonstrating strong age--stage correlations (liver $\rho = 0.971$, muscle $\rho = 0.965$).}
...
\begin{figure}[H]
  ...
  \textbf{a}, Scatter plot of $\log_2$ fold changes between pig and human, showing significant positive correlation (Pearson $r = 0.473$, $p = 6.87 \times 10^{-4}$, 95\% CI: 0.218--0.667). Of 48 genes meeting significance criteria ($p < 0.05$) in both species, 43 (89.6\%) showed conserved direction of change.
