...
Gene expression values typically follow a heavy-tailed distribution, with a small number of highly expressed genes and many genes with low expression \parencite{love_moderated_2014}. Log transformation reduces the skewness of this distribution, making it more suitable for downstream analysis.
...
To obtain unbiased performance estimates while simultaneously optimising hyperparameters, a nested cross-validation design was employed. This approach avoids the optimistic bias that arises when hyperparameters are tuned on the same data used for performance evaluation.
...
Within each outer fold, hyperparameters were optimised using an inner 3-fold cross-validation loop combined with Bayesian optimisation via Optuna \parencite{akiba_optuna_2019}. Each inner loop evaluated 25 candidate configurations using the Tree-structured Parzen Estimator (TPE) sampler with a median pruner for early termination of unpromising trials. The optimisation objective was balanced accuracy on the inner validation folds.
...
Rather than using fixed hyperparameters, nine key parameters were optimised per tissue via Optuna within the inner cross-validation loop (see \autoref{sec:methods:nested_cv}). The search space is summarised in \autoref{tab:hyperparameters}.
...
No explicit feature selection step was applied. Models were trained on all genes passing the detection-rate filter (approximately 20,000--25,000 genes per tissue). LightGBM's built-in regularisation mechanisms---feature subsampling (\texttt{feature\_fraction}), L1 and L2 penalties, and limited tree depth---provide implicit feature selection by assigning zero importance to uninformative genes.
...
The 95\% confidence interval for the Pearson correlation was computed using Fisher's $z$-transformation. The sample correlation $r$ was transformed to $z = \text{arctanh}(r)$, which is approximately normally distributed with standard error $1/\sqrt{n-3}$. The confidence interval was constructed in the $z$-domain and back-transformed to the correlation scale.
...
Three developmental contrasts were defined, all using the Infant stage as the baseline: Infant versus Early childhood, Infant versus Pre-pubertal, and Infant versus Post-pubertal. For each contrast and each gene, the log$_2$ fold change was computed and statistical significance was assessed using the Mann-Whitney $U$ test with Benjamini-Hochberg correction for multiple testing.
