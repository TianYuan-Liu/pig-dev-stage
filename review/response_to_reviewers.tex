...
($n = 8{,}277$, verified at the Ensembl Compara REST homology endpoint;
see also the R3.2 response); $\log_2$(Infant/Adult) in both species
using $\mathrm{log}_2(x+0.01)$ on raw RPKM/TPM (matching the original
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item Brain (strict, $n = 302$): Pearson $r = 0.26$, 95\% CI
        $[0.16, 0.37]$, $p = 3.5 \times 10^{-6}$, directional
        concordance 81.5\%.
  \item Brain (pig-anchored, $n = 2{,}418$): $r = 0.19$, $p = 4.0
        \times 10^{-20}$.
  \item Liver (strict, $n = 0$): no orthologs pass the strict
        bidirectional filter (consistent with the small human cohort).
  \item Liver (pig-anchored, $n = 941$): $r = 0.25$, $p = 2.0 \times
        10^{-14}$, concordance 50.3\%.
\end{itemize}
The brain result is directionally compatible with the muscle finding
($r = 0.69$, $n = 66$) but the magnitude is smaller. We attribute this
to the markedly smaller per-tissue human cohort in Cardoso-Moreira
(brain: $n = 16$; liver: $n = 10$) relative to Schaiter \textit{et~al.}
($n = 11$ but with cleaner perinatal vs adult separation). This is now
stated as a limitation of the extension rather than glossed.

\textbf{Honest framing.} We do not over-claim multi-tissue conservation.
We say: ``The brain result is directionally compatible with the muscle
finding---less complete because of the markedly smaller human cohort,
but already significant---while liver would clearly benefit from a
larger human postnatal RNA-seq cohort.'' Lung remains a known gap and
...
muscle/Sex ($p = 0.005$), muscle/BioProject ($p = 0.014$), and
liver/Breed ($p = 0.008$); Brain/Sex, Liver/Sex, and Lung/Sex are
consistent with chance ($p \geq 0.23$).
